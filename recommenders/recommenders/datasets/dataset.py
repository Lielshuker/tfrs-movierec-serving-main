...
class Dataset:
    ...
    def load_or_generate_data(self, update_to_latest_db=True):
        ...
        self.train = tf.data.experimental.make_csv_dataset(os.path.join(dirname, RATING_TRAIN_FILENAME),batch_size=self.batch_size,num_epochs=5,
                                                           # column_defaults=[tf.string, tf.constant(float('nan'), tf.float32)], na_value='NA',
                                                           column_names=['userid','movieid','rating'
])
        self.test = tf.data.experimental.make_csv_dataset(os.path.join(dirname, RATING_TEST_FILENAME),batch_size=self.batch_size,num_epochs=5,
                                                          column_names=['userid','movieid','rating'])#column_defaults=[tf.string, tf.constant(float('nan'), tf.float32)])
        self.movies = tf.data.experimental.make_csv_dataset(os.path.join(dirname, MOVIE_FILENAME),batch_size=self.batch_size,num_epochs=5,shuffle=False,
                                                            column_names=['movieid','movielensid','imdbid','title','cover','plot','year'
])#column_defaults=[tf.string, tf.constant(float('nan'), tf.float32)])
...
def _load_or_generate_csv_data(test_fraction, update_to_latest_db):
    ...
    with open(os.path.join(datadir,MOVIE_FILENAME), 'w', encoding="utf-8") as f:
